...
public class ConfigApplicationPanel extends JFrame implements ActionListener {
	...
	private static String title = "汉化设置";
	...
	private JLabel sLangLable = new JLabel("原始语言");
	private JComboBox<String> sLangLableVal;

	private JLabel dLangLable = new JLabel("目标语言");
	private JComboBox<String> dLangLableVal;

	// added
	private JLabel fLangLable = new JLabel("文件语言");
	private JComboBox<String> fLangLableVal;

	private JButton detailButton = new JButton("详细设定");
	...
	public ConfigApplicationPanel() {
		...
		addMouseMotionListener(new MouseMotionAdapter() {
			public void mouseDragged(MouseEvent e) {
				...
				ConfigApplicationPanel.this.setLocation(p.x + e.getX() - ConfigApplicationPanel.origin.x,
						p.y + e.getY() - ConfigApplicationPanel.origin.y);
			}
		});
		// 主要面板
		...
		// added
		// 因为新增了这一段,以下各物件的setBounds的第二项都要增加30(往下移)。
		// 为了因应CSV,新增CSV选项。
		...
		this.dLangLableVal.addItem("简体中文");
		this.dLangLableVal.addItem("繁体中文");
		...
		if (Config.getProperty("FLanguage").equals("CSV")
				|| ((String) this.fLangLableVal.getSelectedItem()).equals("CSV")) {
			this.dLangLable.setVisible(false);
			this.dLangLableVal.setVisible(false);
		}

		this.detailButton.setBounds(180, 160, 80, 20);
			...
			if (returnVal == 0) {
				...
				if (isFFXIVFloder(filePath)) {
					...
					JOptionPane.showMessageDialog(null,
							"<html><body>请选择正确的游戏根目录<br />目录预设名为:<br />FINAL FANTASY XIV ONLINE</body></html>", "路径错误",
							0);
				}
			}
		}
